...
export async function GET(req: NextRequest) {
  ...
  // ── Input validation ────────────────────────────────────────────────────────
  if (!roomType || !from || !to) {
    return Response.json({ error: 'Missing params: roomType, from, to' }, { status: 400 });
  }
  ...
  try {
    const availability     = await getAvailabilityForRange(roomType, from, to);
    const unavailableDates = await getUnavailableDates(roomType, from, to, rooms);
    const upgrade = unavailableDates.length > 0
